...
if (__name__ == "__main__"):
 ...
 #   print(build_ngram_model(ngram_counts, 3))
...
'''
    ...
    for line in lines: 
        ...
        for word in line:
            if (word.isalpha()):
                ...
                l = len(word)
                for letter in word:
                    if(letter in utilities.BAD_CHARS):         # removes bad chars generally
                        word.replace(letter, '')
                    
                    elif((letter in utilities.END_OF_SENTENCE_PUNCTUATION) and (letter != word[l-1])):     #removes for end words in middle
                        word.replace(letter, '')

                    elif(letter in utilities.VALID_PUNCTUATION):        #considers good punctuation
                        t.append(letter)
                        word.replace(letter, '' + letter + '')
                f2.append(word)
                if(t):
                    f2.extend(t)     # reconstructs new line f2
                print(f2, "in")
        print(f2, "out")
   ...
   keys = [k for k in counts]
    ...
    for i in range(len(keys)):
        ctr = 0
        temp = []
        while( (ctr <= len(count_nums[i]))):

            k = help_count[i].index(max(help_count[i]))
            print(k)
            temp.append(k)
            help_count[i].pop(k)
            help_pred[i].pop(k)

            if(ctr > prune_len - 1 and (help_count[i][k] != help_count[i][temp[-1]] )):
                temp.pop(k)
                break 
        ...
        print(new_indexes)
        new_count.append( [count_nums[i][k] for k in new_indexes[i]])
        new_pred.append( [pred_list[i][k] for k in new_indexes[i]] ) 


    final = {keys[j]:[new_pred[j], new_count[j]] for j in range(len(keys)) 
    }
    return final
